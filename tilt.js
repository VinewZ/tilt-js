...
function tiltOnMouseEnter(elementId, opt) {
  ...
  const options = {
    ...
    maxTiltX: (opt == null ? void 0 : opt.xNegative) ? -((opt == null ? void 0 : opt.maxTiltX) ?? 30) : (opt == null ? void 0 : opt.maxTiltX) ?? 30,
    maxTiltY: (opt == null ? void 0 : opt.yNegative) ? (opt == null ? void 0 : opt.maxTiltY) ?? 30 : -((opt == null ? void 0 : opt.maxTiltY) ?? 30)
  };
  htmlElement == null ? void 0 : htmlElement.addEventListener("mousemove", (event) => {
    const elementWidth = htmlElement.offsetWidth;
    const elementHeight = htmlElement.offsetHeight;
    const centerX = htmlElement.offsetLeft + elementWidth / 2;
    const centerY = htmlElement.offsetTop + elementHeight / 2;
    const mousePosX = event.clientX - centerX;
    const mousePosY = event.clientY - centerY;
    const rotateX = options.maxTiltX * mousePosY / (elementHeight / 2);
    const rotateY = options.maxTiltY * mousePosX / (elementWidth / 2);
